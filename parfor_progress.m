...
if N > 0
    ...
    if nargout == 0
        num = sprintf('%6d/%6d', 0,N);
        disp(['  0%[>', repmat(' ', 1, w), '] ',num]);
    end
elseif N == 0
    ...
    if nargout == 0
        ...
        num = sprintf('%6d/%6d', progress(1),progress(1));
        disp([repmat(char(8),1,(w+9+length(num)+1)),newline,perc,'[',repmat('=',1,...
            round(percent*w/100)),'>',repmat(' ',1,w-round(percent*w/100)),'] ',num]);
    end
else
    ...
    if nargout == 0
        perc = sprintf('%3.0f%%', percent); % 4 characters wide, percentage
        num = sprintf('%6d/%6d', length(progress)-1,progress(1));
        disp([repmat(char(8),1,(w+9+length(num)+1)),newline,perc,'[',repmat('=',1,...
            round(percent*w/100)),'>',repmat(' ',1,w-round(percent*w/100)),'] ',num]);
    end
end
